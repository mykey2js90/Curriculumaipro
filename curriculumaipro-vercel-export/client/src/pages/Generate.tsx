...
export default function Generate() {
  ...
  const downloadJSON = () => {
    ...
    a.href = url;
    a.download = curriculum.courseTitle.replace(/\s+/g, "_") + ".json";
    a.click();
  ...
  const downloadCSV = () => {
    if (!curriculum) return;
    const rows = [
      ["Module", "Title", "Description", "Duration", "Topics", "Objectives"],
    ];
    curriculum.modules.forEach(m => {
      rows.push([
        String(m.moduleNumber),
        m.title,
        m.description,
        m.estimatedDuration,
        m.topics.join("; "),
        m.learningObjectives.join("; "),
      ]);
    });
    const csv = rows
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = curriculum.courseTitle.replace(/\s+/g, "_") + ".csv";
    a.click();
    URL.revokeObjectURL(url);
  };
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <h1 className="text-2xl font-bold mb-2">
              Sign in to Generate Curricula
            </h1>
            <p className="text-muted-foreground mb-6">
              Create a free account to start generating AI-powered course
              curricula.
            </p>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Enter a topic and let AI build a complete course structure for you.
          </p>
        </div>
              ...
              <CardContent className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    step={1}
                    value={[numModules]}
                    onValueChange={([v]) => setNumModules(v)}
                  />
                </div>
                <Button
            ...
            {curriculum ? (
                    ...
                    {curriculum.prerequisites.length > 0 && (
                      <div className="mt-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {curriculum.prerequisites.map(p => (
                            <Badge
                              key={p}
                              variant="outline"
                              className="text-xs"
              ...
              <div className="h-full flex items-center justify-center border-2 border-dashed border-border rounded-xl p-12 text-center">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Enter a topic and click Generate to create your curriculum.
                  </p>
                </div>
              </div>
            )}
