...
Respond ONLY with valid JSON.`;
          ...
          messages: [
            {
              ...
              content:
                "You are an expert instructional designer. Generate structured course curricula in JSON.",
            } as Message,
  ...
  sponsors: router({
    ...
    recordImpression: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        await recordSponsorImpression(input.id);
        return { success: true };
      }),
    recordClick: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        await recordSponsorClick(input.id);
        return { success: true };
      }),
    analytics: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new Error("Forbidden");
